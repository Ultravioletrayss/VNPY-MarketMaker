# csv_data_reader.py - CSV数据读取测试脚本
# 用于读取和显示期货合约CSV文件中的交易数据
...
# 当前脚本只导入 rb2505
TARGET_SYMBOL = "rb2505"
TARGET_EXCHANGE = Exchange.SHFE

# 当前要导入的 CSV 文件路径
CSV_FILE_PATH = r"C:\Users\ultra\Desktop\20250303\rb2505.csv"
    ...
    with f:
        for row in reader:
            ...
            if symbol != TARGET_SYMBOL:
                raise ValueError(
                    f"CSV 里的 InstrumentID 是 {symbol}，"
                    f"但当前脚本只允许导入 {TARGET_SYMBOL}"
                )
...
def delete_old_tick_data() -> None:
    """
    只删除 rb2505.SHFE 的旧 TickData。
    ...
    try:
        count = database.delete_tick_data(TARGET_SYMBOL, TARGET_EXCHANGE)
        print(
            f"已删除旧 TickData："
            f"{TARGET_SYMBOL}.{TARGET_EXCHANGE.value}，数量：{count}"
        )
...
def main() -> None:
    ...
    # 只删除 rb2505.SHFE 的旧数据
    delete_old_tick_data()
